...
select count(hasProfilePicture) user_with_ProfilePicture from users_data where hasProfilePicture = 'true';
-- b.Uses Application for Ecommerce platform
select count(hasAnyApp) user_with_hasAnyapp from users_data where hasAnyApp = 'true';
-- c.Uses Android app
select count(hasAndroidApp) user_with_AndroidApp from users_data where hasAndroidApp = 'true';
-- d.Uses ios app
select count(hasIosApp) user_with_IOSApp from users_data where hasIosApp = 'true';
-- 8.Calculate the total number of buyers for each country and sort the result in 
-- descending order of total number of buyers. 
...
-- 9.Calculate the total number of sellers for each country and sort the result in 
...
--  wishlist or to like socially on an ecommerce platform. (Hint: use UNION to answer this question.)
select sum(productsWished) ,sum(socialProductsLiked) from users_data where gender='F';
# choice of female users about putting the product in a wishlist or to like socially on
# an ecommerce platform is same. 
...
select country, max(productsBought) Max_no_of_Buyers from users_data group by country order by max(productsBought) desc limit 1;
 #Belgique country has the maximum no.of buyers
